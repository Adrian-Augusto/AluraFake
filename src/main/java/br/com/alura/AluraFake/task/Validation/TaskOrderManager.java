...
public class TaskOrderManager {
    ...
    public void prepararParaInsercaoText(List<NewOpenText> tasks, Integer novaOrdem) {
    ...
    public void inserirTaskNaOrdemText(List<NewOpenText> tasks, NewOpenText novaTask, Integer novaOrdem) {
        // 1. Reordenar a lista
        tasks.sort(Comparator.comparingInt(Task::getOrderr));
        for (int i = 0; i < tasks.size(); i++) {
            tasks.get(i).setOrderr(i + 1);
